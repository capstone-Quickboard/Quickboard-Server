...
public class NoticeController {
    ...
    **/
    ...
    public BaseResponse<SliceResDto<NoticeListResDto>> findAllByDepartment(@RequestParam Long departmentId,
        ...
        try{
            ...
            if(!userService.checkUuidExist(uuid))
                throw new BaseException(NOT_FOUND_USER);
            return new BaseResponse<>(noticeService.findAllByDepartmentId(uuid,departmentId,lastNoticeId, pageable));
        }catch (BaseException exception){
            return new BaseResponse<>(exception.getStatus());
        }
    }
    ...
    public BaseResponse<SliceResDto<BookmarkedNoticeListResDto>> findAllByBookmarked(@RequestParam(required = false) Long lastNoticeId, Pageable pageable){
        try{
            String uuid = uuidService.getUuid(); //Header에서 uuid 받아옴
            if(!userService.checkUuidExist(uuid))
                throw new BaseException(NOT_FOUND_USER);
            return new BaseResponse<>(noticeService.findAllByBookmarked(uuid, lastNoticeId, pageable));
        }catch (BaseException exception){
            return new BaseResponse<>(exception.getStatus());
        }
    }
